...
def _truck_override_for_path(path: Path) -> str | None:
...
def ingest_and_embed_all(raw_dir: Path | None = None) -> dict:
...
def bootstrap(reseed: bool = True, raw_dir: Path | None = None) -> dict:
    init_db()
    with get_db() as db:
        if reseed:
