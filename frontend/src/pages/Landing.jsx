...
import { fetchTruckingNews } from "../services/news.js";
...
const TAG_COLOR = {
  Fuel:       { bg: "#FEF3C7", color: "#92400E" },
  Regulation: { bg: "#DBEAFE", color: "#1D4ED8" },
  Safety:     { bg: "#FEE2E2", color: "#B91C1C" },
  Tech:       { bg: "#EDE9FE", color: "#6D28D9" },
  Market:     { bg: "#DCFCE7", color: "#15803D" },
  Industry:   { bg: "#F1EFE8", color: "#5F5E5A" },
...
export default function Landing() {
  ...
  useEffect(() => {
    fetchTruckingNews().then(data => { setNews(data); setLoading(false); });
    const timer = setInterval(() => setSlide(i => (i + 1) % SLIDES.length), 5000);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7F6F3", fontFamily: "Inter,sans-serif" }}>
      ...
      {/* Main grid */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20, padding: "28px 40px", maxWidth: 1100, margin: "0 auto" }}>

        {/* Left — carousel */}
        <div>
          <div style={{ background: s.bg, borderRadius: 16, padding: "60px 40px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", minHeight: 380, transition: "background .5s" }}>
            ...
            <button onClick={() => setSlide((slide - 1 + SLIDES.length) % SLIDES.length)} style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,.15)", border: "none", color: "#fff", borderRadius: "50%", width: 36, height: 36, fontSize: 20, cursor: "pointer" }}>‹</button>
            <button onClick={() => setSlide((slide + 1) % SLIDES.length)} style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,.15)", border: "none", color: "#fff", borderRadius: "50%", width: 36, height: 36, fontSize: 20, cursor: "pointer" }}>›</button>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "center", marginTop: 12 }}>
            <div style={{ color: "#9C9890", fontSize: 20 }}>↓</div>
          </div>
        ...
        <div>
          <div style={{ background: "#fff", border: "1px solid #E4E2D9", borderRadius: 16, overflow: "hidden" }}>
            <div style={{ padding: "12px 18px", borderBottom: "1px solid #E4E2D9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 15 }}>📰</span>
                <span style={{ fontSize: 14, fontWeight: 600 }}>Trucking Industry News</span>
              </div>
              {!loading && (
                <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 99, background: "#DCFCE7", color: "#15803D" }}>● Live</span>
              )}
            </div>

            {loading ? (
              <div style={{ padding: "32px", textAlign: "center", color: "#9C9890", fontSize: 13 }}>
                <div style={{ fontSize: 24, marginBottom: 8 }}>📡</div>
                Fetching latest trucking news…
              </div>
            ) : (
              news.map((n, i) => {
                const tc = TAG_COLOR[n.tag] || TAG_COLOR.Industry;
                return (
                  <a key={i} href={n.url} target="_blank" rel="noreferrer" style={{ display: "block", padding: "14px 18px", borderBottom: i < news.length - 1 ? "1px solid #F0EEE8" : "none", textDecoration: "none", color: "inherit", transition: "background .15s" }}
                    onMouseEnter={e => e.currentTarget.style.background = "#FAFAF8"}
                    onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                      <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 99, background: tc.bg, color: tc.color, flexShrink: 0 }}>{n.tag}</span>
                      <span style={{ fontSize: 11, color: "#9C9890", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{n.source} · {n.date}</span>
                    </div>
                    <div style={{ fontSize: 13, fontWeight: 600, lineHeight: 1.4, marginBottom: n.snippet ? 4 : 0 }}>{n.title}</div>
                    {n.snippet && <div style={{ fontSize: 12, color: "#6B6965", lineHeight: 1.5 }}>{n.snippet}</div>}
                  </a>
                );
              })
            )}
          ...
          <div style={{ marginTop: 10, fontSize: 11, color: "#9C9890", textAlign: "center" }}>
            Powered by Tavily · Updates on page load
          </div>
        </div>
      </div>
